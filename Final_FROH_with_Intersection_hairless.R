# Binary Trait : Hairless

library(dplyr)
library(ggplot2)

datahairless <- read.csv("C:/Users/Sweetalana/OneDrive - The Pennsylvania State University/Desktop/Research Lab/722_Breeds/CSV_XLS_files/Final_phenotypes_values/Binary_Traits/Hairless.csv")
...
result_dfB <- AllIndividuals_Intersection_without_outliers %>%
  semi_join(datahairless, by = c("V1" = "id"))

result_dfB1 <- result_dfB %>% semi_join(datahairless,by=c("V2" ="id"))
...
library(GMMAT)  
all_data <- datahairless

#Hairless

model1 <- glmmkin(HairlessAdjusted ~ FROH, data = all_data
...
LogBeta1 #1.636862 
LogBetaStandardized1 #0.3259026    
LogisticPval1 #0.7444981    
confintUpper1 # Intercept -2.726964      with FROH  11.481061     
confintLower1 # Intercept -8.936500     with FROH -8.207336    

confintUpper1[2] # 11.48106     
confintLower1[2] #-8.207336  

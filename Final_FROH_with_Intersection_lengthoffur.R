# Binary Trait : lengthoffur

library(dplyr)
library(ggplot2)

datalengthoffur <- read.csv("C:/Users/Sweetalana/OneDrive - The Pennsylvania State University/Desktop/Research Lab/722_Breeds/CSV_XLS_files/Final_phenotypes_values/Binary_Traits/Lengthoffur.csv")
...
result_dfB <- AllIndividuals_Intersection_without_outliers %>%
  semi_join(datalengthoffur, by = c("V1" = "id"))

result_dfB1 <- result_dfB %>% semi_join(datalengthoffur,by=c("V2" ="id"))
...
library(GMMAT)  
all_data <- datalengthoffur

#Lengthoffur

model1 <- glmmkin(LengthoffurrAdjusted ~ FROH, data = all_data
